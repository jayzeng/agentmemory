...
## Adding a New Command (Required)

When adding a new subcommand to `src/cli.ts`, you **must** also update all four of the following in the same commit:

1. **`src/cli-spec.ts` — `COMMANDS` array**: add the new command name.
2. **`src/cli-spec.ts` — `COMMAND_DESCRIPTIONS`**: add a one-line description.
3. **`src/cli-spec.ts` — `COMMAND_OPTIONS`**: list every flag the command accepts (even if empty `[]`).
4. **`src/cli-spec.ts` — `OPTION_SPECS`**: add an entry for every new flag with its description and value kind.

Shell completions (Bash, Zsh, Fish, PowerShell) are generated **directly from `cli-spec.ts`** at runtime — no separate completion file to edit. Verify the result with:

```bash
bun src/cli.ts completion --stdout | grep '<new-command>'
bun src/cli.ts help <new-command>
```

Both must show the command and its flags.

...
- Cursor now gets event-driven capture through native `beforeSubmitPrompt`, `afterFileEdit`, shell/MCP completion, and `stop` hooks. The implementation avoids undocumented transcript parsing, stores only bounded hashed per-conversation capture state, emits Cursor-native `followup_message`, and raises the strict local measured-harness capture baseline from 75% to 100%.

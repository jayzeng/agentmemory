...
- **AgentMemory Pro bootstrap failed:** doctor will show a `fail` row with the underlying error. Re-run `agent-memory pro install`; if it persists, core functionality (write/read/scratchpad/search/hooks) is entirely unaffected — Pro failures are isolated by design.

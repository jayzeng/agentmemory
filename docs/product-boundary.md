...
- Keeps coding history, memories, queries, repository paths, and raw session identifiers on the device.
...
Paying will remove limits; it will not unlock access to your own local data. Low-level `agent-memory plugin` commands remain available for administration and scripting. See [how local privacy, signing, installation, and capability enforcement work](official-plugin-bootstrap.md).
